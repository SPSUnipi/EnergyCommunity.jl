...
```math
P^{U+}_{j,t} - P^{U-}_{j,t} + \sum_{c\in A^C_j}(P^{c-}_{j,t} - P^{c+}_{j,t}) - P^R_{j,t} + \sum_{g \in A^G_j}P^g_{j,t} + \sum_{d \in A^D_j}P^{\text{adj}}_{j,d,t} + \sum_{hp \in A^{hp}_j}P^{HP}_{j,hp,t} = - \sum_{f \in A^F_j} P^L_{j,f,t}
```
...
- ``P^{HP}_{j,hp,t}`` is the power consumed by heat pump ``hp``;
